import { Camera, Mesh, Vec, Model, Color, PointLight } from './3d.js';
import { debugToggles, setupDebugToggles } from './debug_toggles.js';
import { getProgramVarLocations, loadProgramFromFiles } from './gl.js';
import { Block, MCWorld } from './mc.js';
import { atlasImages, loadImages } from './texture.js';
...
async function main() {
    ...
    const glStuff = { ...gl1or2, program: programInfo };
    ...
    const stuff = { glStuff, camera, world, atlas, input: inputInfo, light };
    setupUI(stuff);
    ...
    function mouseMoveDuringPointerLock(ev) {
    ...
    function mousedownDuringPointerLock(ev) {
    ...
    function mouseupDuringPointerLock(ev) {
    ...
    function keydownDuringPointerLock(ev) {
        ev.preventDefault();
        if (ev.repeat)
            return;
    ...
    function keyupDuringPointerLock(ev) {
        ev.preventDefault();
...
function setupWebGL({ gl }) {
    ...
    if (!debugToggles.has('no_backface_culling'))
        gl.enable(gl.CULL_FACE);
    // gl.enable(gl.BLEND);
    // gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
}
const SHADER_ATTRIBUTE_NAMES = ['a_Position', 'a_UV', 'a_Normal'];
const SHADER_UNIFORM_NAMES = ['u_ModelMat', 'u_CameraMat', 'u_NormalMat', 'u_TextureAtlas', 'u_MaxTextureAtlasLOD', 'u_TextureAtlasDimensions', 'u_Color', 'u_LightColor', 'u_LightPos'];
function setupGLExtensions({ gl }) {
    var _a;
    const supportedExtensions = (_a = gl.getSupportedExtensions()) !== null && _a !== void 0 ? _a : [];
...
async function setupShaders({ gl, hasWebgl2 }, extensions) {
    const hasExtensionDefines = [];
    if (hasWebgl2)
        hasExtensionDefines.push('HAS_WEBGL2');
...
function setupTextures(atlas, { gl, hasWebgl2, program: { uniform: { u_TextureAtlas, u_MaxTextureAtlasLOD, u_TextureAtlasDimensions } } }) {
    ...
    if (u_MaxTextureAtlasLOD !== null)
        gl.uniform1f(u_MaxTextureAtlasLOD, atlas.maxMipLevel);
    if (u_TextureAtlasDimensions !== null)
        gl.uniform2f(u_TextureAtlasDimensions, atlas.width, atlas.height);
    //
...
async function serverTick(stuff) {
...
function renderTick(stuff, now) {
    ...
    if (heldKeys.has('KeyW')) {
        posDelta.addInPlace(Vec.fromPolarXZ(delta / 100, camera.rotX));
    }
    if (heldKeys.has('KeyS')) {
        posDelta.addInPlace(Vec.fromPolarXZ(delta / 100, camera.rotX + Math.PI));
    }
    if (heldKeys.has('KeyA')) {
        posDelta.addInPlace(Vec.fromPolarXZ(delta / 100, camera.rotX - Math.PI / 2));
    }
    if (heldKeys.has('KeyD')) {
        posDelta.addInPlace(Vec.fromPolarXZ(delta / 100, camera.rotX + Math.PI / 2));
    }
    if (heldKeys.has('Space')) {
        posDelta.y += delta / 100;
    }
    if (heldKeys.has('ShiftLeft')) {
        posDelta.y -= delta / 100;
    }
    if (heldKeys.has('KeyQ')) {
        camera.rotX -= delta / 100 / 5;
    }
    ...
    if (u_LightColor !== null)
        gl.uniform4f(u_LightColor, light.color.r, light.color.g, light.color.b, light.color.a);
    if (u_LightPos !== null)
        gl.uniform3fv(u_LightPos, light.pos.xyz());
