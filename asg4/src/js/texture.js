...
import { assert, isPow2, setFind, warnRateLimited } from "./util.js";
...
export async function loadImages(names) {
    return await Promise.all(names.map(async (name) => [name, await loadImgFromPath(texName2Path(name))]));
}
export function atlasImages(images, initialAtlasSize = 128) {
...
function atlasImages_(images, atlasSize, margin = 16) {
    var _a;
    ...
    {
        const fullSizeAtlasCtx = (_a = mipCanvases.find(([level]) => level === 0)) === null || _a === void 0 ? void 0 : _a[2];
