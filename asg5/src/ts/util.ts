...
export function getElementByIdAndValidate<T extends HTMLElement>(id: string, { document: doc, type }: { document?: Document | undefined, type?: {new(): T} } = {}): T {
    const el = (doc ?? document).getElementById(id);
    assert(el !== null, `found no element with id #${id}`);
    if(type !== undefined) assert(el instanceof type);
    return el as T;
}
